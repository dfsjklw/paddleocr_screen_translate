...
- **覆盖层配置** — 支持背景不透明度、背景颜色、文字颜色、字体大小等实时调节
...
- **全部按钮** — 开始、暂停、停止、单次翻译、划屏翻译、清除覆盖层、按住隐藏等
- **选项卡标签** — 控制、OCR、翻译器、覆盖层、采集、快捷键、日志、结果
- **状态栏信息** — 运行状态、周期统计（文本框数、耗时等）
- **OCR 引擎信息** — 引擎名称、模型路径
- **翻译器参数** — 所有字段标签、提示文字
- **覆盖层设置** — 字号、透明度、字体、文字颜色、背景颜色等
- **快捷键设置** — 所有快捷键字段标签
- **划屏选择器** — 操作提示、窗口标题
- **对话框消息** — 错误提示、警告、连接测试结果
- **系统托盘** — 显示/隐藏、退出菜单
- **日志区域** — 所有标签和提示文字

中文/英文切换后即时生效，无需重启应用。所有 UI 文本定义在 `src/python/i18n/strings.py` 中，新增翻译键需同时提供 `"en"` 和 `"zh"` 两个语言版本。
